...
const is_web = @import("main.zig").is_web;
...
pub fn run() void {
    LevelState.init();
    defer LevelState.deinit();
    if (is_web) {
        emscripten.emscripten_set_main_loop(frame, 0, 0);
    } else {
...
fn frame() callconv(.c) void {
    switch (_frame()) {
        .continue_game => {},
        .restart_game => LevelState.init(),
    }
}
fn _frame() enum { continue_game, restart_game } {
    core.t += 1;
    inputs.updateButtonsHeld();
    inputs.updateGamepadConnections();

    const SCORE_MAX_DIGITS = std.fmt.comptimePrint("{}", .{std.math.maxInt(@TypeOf(level.score))}).len;
    ...
    const show_restart_button = level.game_over_t != null and level.scoring_hexagons.items.len == 0;
    var restart_button_is_hovered = false;

    // Update
    {
        level.time_left -= FRAME_DELTA;
        if (level.time_left <= 0) {
            level.time_left = 0;
            if (level.game_over_t == null) level.game_over_t = core.t;
        }
        ...
        if (level.game_over_t == null) update_shapes: {
            level.wrong_choice_timer -= FRAME_DELTA;
            if (level.wrong_choice_timer <= 0) {
                level.wrong_choice_timer = 0;
            ...
            inline for (&level.grid, 0..) |*column, grid_x| {
                inline for (column, 0..) |*shape, grid_y| cont: {
                    ...
                    const a_wrong_choice_was_made = level.wrong_choice_timer > 0;
            ...
            clicked.effect = .none;
            const other_pos = level.selected_square orelse {
                // None already selected
                switch (clicked.kind) {
                    .empty => @panic("clicked empty square"),
                    .square, .diamond => {
                        level.selected_square = clicked_pos;
                    },
                    .hexagon => {
                        clearHexagonColorChain(clicked_pos, 1);
                        level.time_left += @floatFromInt(level.chain_clear_time_bonus);
                        level.chain_clear_time_bonus = @max(level.chain_clear_time_bonus / 2, 1);
                        level.time_bonus_flicker_left = 0.5;
                    },
                }
                break :update_shapes;
            };
            level.selected_square = null;
    ...
    rl.beginTextureMode(level.game_texture);
    {
        ...
        inline for (level.grid, 0..) |column, grid_x| {
            inline for (column, 0..) |shape, grid_y| {
                ...
                const bg_color: Color =
                    if (level.selected_square == grid_pos)
                        Color.white
                ...
                drawShape(shape, center, .{
                    .scale = if (level.game_over_t) |got|
        ...
        {
            {
                ...
                const text = std.fmt.bufPrintZ(&buffer, "Score: {}", .{level.score}) catch unreachable;
                _ = draw.textHighRes(
                    text,
            ...
            }
            {
                ...
                const time_text = std.fmt.bufPrintZ(&time_buffer, "Time: {d:.0} ", .{level.time_left}) catch unreachable;
                x += draw.textHighRes(
                    time_text,
                    x,
                    top,
                ...
                level.time_bonus_flicker_left = @max(level.time_bonus_flicker_left - FRAME_DELTA, 0);
                // When flickering, alternate on/off every 0.1 seconds.
                if (level.time_bonus_flicker_left == 0 or @as(u32, @trunc(level.time_bonus_flicker_left * 10)) % 2 == 0) {
                    var bonus_buffer: ["Clear bonus: 20".len + 1]u8 = undefined;
                    const bonus_text = std.fmt.bufPrintZ(&bonus_buffer, "Clear bonus: {}", .{level.chain_clear_time_bonus}) catch unreachable;
                    _ = draw.textHighRes(
                        bonus_text,
                        x,
                        top + 9,
    ...
    rl.beginDrawing();
    {
        ...
        level.bg_hue += 60.0 / 2 * FRAME_DELTA;
        rl.clearBackground(Color.fromHSV(level.bg_hue, 0.5, 0.5));
...
pub const LevelState = struct {
    ...
    merging_squares: std.ArrayList(Merging) = .initBuffer(&_merging_squares_buffer),
    scoring_hexagons: std.ArrayList(Scoring) = .initBuffer(&_scoring_hexagons_buffer),

    /// Custom animations. Automatically removed when the anim is done, but otherwise
    /// controlled by whatever code added them.
    vfx: std.ArrayList(Entity) = .initBuffer(&_vfx_buffer),
    ...
    sfx: std.ArrayList(rl.Sound) = .initBuffer(&_sfx_buffer),

    game_over_t: ?u32 = null,

    score: u64 = 0,
    chain_clear_time_bonus: u64 = 20,
    time_bonus_flicker_left: f32 = 0,
    time_left: f32 = 30.0,

    bg_hue: f32,

    selected_square: ?GridPosition = null,
    wrong_choice_timer: f32 = 0,
    ...
    var _merging_squares_buffer: [10]LevelState.Merging = undefined;
    var _scoring_hexagons_buffer: [COLUMN_COUNT * ROW_COUNT]LevelState.Scoring = undefined;
    var _vfx_buffer: [10]Entity = undefined;
    var _sfx_buffer: [10]rl.Sound = undefined;
